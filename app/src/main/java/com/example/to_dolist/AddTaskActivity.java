...
public class AddTaskActivity extends AppCompatActivity {
            ...
            if (mTaskId == DEFAULT_TASK_ID) {
                ...
                viewModel.getTask().observe(this, new Observer<TaskEntry>() {
                    ...
                    public void onChanged(TaskEntry taskEntry) {
                        // We are not going to listen to any updates to the database in this
                        // activity, that is why we are going to remove the observer
    ...
    public void onSaveButtonClicked() {
        // Getting the values stored in the fields
        String description = mEditText.getText().toString();
        int priority = getPriorityFromViews();
        Date date = new Date();

        final TaskEntry taskEntry = new TaskEntry(description, priority, date);
        // Executing insert() in another thread
        AppExecutors.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                // Inserting the taskEntry object into the database
                if (mTaskId == DEFAULT_TASK_ID) {
